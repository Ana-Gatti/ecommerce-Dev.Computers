...
export class DetalhesProdutosComponent implements OnInit {
  ...
  ngOnInit(): void {
    const routeParams = this.route.snapshot.paramMap; //para mapear todos os parâmetros da rota
    const produtoId = Number(routeParams.get("id")); //pegar um produto por ID
